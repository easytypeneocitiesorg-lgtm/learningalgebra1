...
function updateUIAfterLogin(username) {
  ...
  // Unified Owner Tools
  if (isOwner) {
    document.getElementById('owner-action-btns').classList.remove('hidden');
  } else {
    document.getElementById('owner-action-btns').classList.add('hidden');
  }
}
...
document.getElementById('admin-search-btn').addEventListener('click', async () => {
  ...
  if(snap.exists() && !snap.val().wipedTo) {
    ...
    // Check if blocked only
    const blockSnap = await get(child(dbRef, `blocked_users/${search}`));
    if(blockSnap.exists()) {
      adminTargetUser = search; adminTargetUserData = null;
      document.getElementById('target-user-display').textContent = `Target: @${search} (Blocked Only)`;
      document.getElementById('admin-user-actions').classList.remove('hidden');
    } else {
...
function canPerformActionOnTarget() {
  ...
  if (adminTargetUser === 'owner' && currentActiveUser !== 'owner') return (alert("Denied: Cannot target site owner."), false);
  if (currentActiveUser !== 'owner' && (adminTargetUser === 'owner' || (adminTargetUserData && adminTargetUserData.isStaff))) return (alert("Denied: Cannot target other staff/owner."), false);
  return true;
}
...
// Owner Only Role Buttons in Admin
document.getElementById('admin-grant-staff-btn').addEventListener('click', async () => {
  if(currentActiveUser !== 'owner' || !adminTargetUser) return;
  await update(ref(db, `users/${adminTargetUser}`), { isStaff: true, isHelper: false });
  alert(`Granted Staff to @${adminTargetUser}`);
});
document.getElementById('admin-grant-helper-btn').addEventListener('click', async () => {
  if(currentActiveUser !== 'owner' || !adminTargetUser) return;
  await update(ref(db, `users/${adminTargetUser}`), { isHelper: true, isStaff: false });
  alert(`Granted Helper to @${adminTargetUser}`);
});
document.getElementById('admin-revoke-btn').addEventListener('click', async () => {
  if(currentActiveUser !== 'owner' || !adminTargetUser) return;
  await update(ref(db, `users/${adminTargetUser}`), { isStaff: false, isHelper: false });
  alert(`Revoked roles from @${adminTargetUser}`);
});
document.getElementById('admin-unblock-btn').addEventListener('click', async () => {
  if(currentActiveUser !== 'owner' || !adminTargetUser) return;
  await remove(ref(db, `blocked_users/${adminTargetUser}`));
  alert(`Unblocked @${adminTargetUser}`);
});
...
messagesContainer.addEventListener('click', async (e) => {
  ...
  if (e.target.classList.contains('admin-block-btn')) {
    ...
    if (targetUsername === 'owner' && currentActiveUser !== 'owner') return alert("Cannot block site owner.");
  ...
  // Quick Action Owner Buttons directly on messages
  if (e.target.classList.contains('owner-grant-staff-btn')) {
    const targetUsername = e.target.dataset.username;
    if(confirm(`Grant Staff to @${targetUsername}?`)) await update(ref(db, `users/${targetUsername}`), { isStaff: true, isHelper: false });
  }
  if (e.target.classList.contains('owner-grant-helper-btn')) {
    const targetUsername = e.target.dataset.username;
    if(confirm(`Grant Helper to @${targetUsername}?`)) await update(ref(db, `users/${targetUsername}`), { isHelper: true, isStaff: false });
  }
});
...
document.querySelectorAll('.channel[data-type="text"]').forEach(el => {
  el.addEventListener('click', () => {
    ...
    if(targetChannel === 'staff' && !(currentUserData.isStaff || currentUserData.isHelper || currentActiveUser === 'owner')) {
...
function switchChannel(channelName, type, extraData = null) {
  ...
  if (type === 'text') {
    ...
    if(channelName === 'rules' && !(currentUserData.isStaff || currentUserData.isHelper || currentActiveUser === 'owner')) {
...
function loadMessages() {
  ...
  unsubscribeMessages = onValue(msgQuery, (snapshot) => {
    ...
    snapshot.forEach((childSnapshot) => {
      ...
      } else if (currentUserData.isStaff || currentUserData.isHelper || currentActiveUser === 'owner') {
        actionBtns += `<button class="delete-btn" data-id="${msgId}">Delete</button>`;
      }

      if ((currentUserData.isStaff || currentActiveUser === 'owner') && !data.isStaff && data.username !== currentActiveUser) {
      ...
      // Inject quick-actions onto messages for Owner
      if (currentActiveUser === 'owner' && data.username !== currentActiveUser) {
        actionBtns += `<button class="owner-grant-staff-btn primary-btn" data-username="${data.username}" style="background-color:var(--success); padding:4px 6px;">+Staff</button>`;
        actionBtns += `<button class="owner-grant-helper-btn primary-btn" data-username="${data.username}" style="background-color:#3b82f6; padding:4px 6px;">+Helper</button>`;
      }
